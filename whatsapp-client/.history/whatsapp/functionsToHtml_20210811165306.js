...
const urlMessages = "http://localhost:8080/api/v1/messages/"
const urlMessagesGroup = "http://localhost:8080/api/v1/messagesGroups/"
const urlUsers = "http://localhost:8080/api/v1/users/";
const urlGroups = "http://localhost:8080/api/v1/group/"
...
function addGroupName(){    
    ...
    if(textName !=""){
        ...
        routerFunctions("POST",`${urlGroups}`,GETA,dataJson);
    }
}
...
function getCliceGroup(name){
    nameGroup = name
    routerFunctions("GET",`${urlMessagesGroup}getMessagesByGroup?group=${name}`,getAllMessagesOfGroups);
...
function getCliceUser(userName,index) {
    ...
    routerFunctions("GET", `${urlMessages}getMessages?sender=${currentUser}&reciver=${userName}`, getAllMessagesByUser)  

    reciver = userName;
    nameReciver =document.getElementById(`spanName${index}`).innerHTML
    console.log("nameeeee",nameReciver);
...
function sendMessage() {
    ...
    routerFunctions("POST", `${urlMessages}`, getAllMessagesByUser, dataJson)
    sendText.value = "";
}
...
function getAllMessages(sender){    
    routerFunctions("GET",`${urlMessages}getMessagesOfSender?sender=${sender}`,getAllMessagesOfSender);
}


function getAllGroups(){        
    routerFunctions("GET",`${urlGroups}?sender=${currentUser}`,getAllGroupsValues);
}

function getMessagesAfterDelete(){
    routerFunctions("GET", `${urlMessages}getMessages?sender=${currentUser}&reciver=${reciver}`, getAllMessagesByUser) 
}



 function removeMessage(id , currentUser){
    console.log(currentUser,reciver);
     routerFunctions("DELETE",`${urlMessages}?id=${id}&sender=${currentUser}`,getMessagesAfterDelete);
}

function removeMessageGroup(id , currentUser , nameGroup){
    routerFunctions("DELETE",`${urlMessagesGroup}?id=${id}&sender=${currentUser}`,getAllMessagesOfGroups);
}

function addUserGroup(id){
    let dataJson = { "nameGroup": nameGroup, "userName": id, "creator": currentUser }
    routerFunctions("POST", `${urlGroups}addUser`, printResult, dataJson)
}
...
function deleted(id){
    ...
    if(select){
        ...
        removeMessage(`${id}`,currentUser,reciver) ;
}
}
...
function writeToHtmlUsers() {
    ...
    for (let i = 0; i < users.length; i++) {
        ...
        <div class="msgName">
            <span id="spanName${i}" style="color:red ;padding-right: 20px;">${messages[i].receiverName}</span>
            <div class="time">
